...
module.exports = async (req, res) => {
    ...
    async function collectUnique(urls) {
        ...
        settled.forEach((result) => {
            ...
            result.value.forEach((anime) => { if (!anime.slug || seen.has(anime.slug)) return; seen.add(anime.slug); unique.push(anime); });
        });
        return unique;
    }

    function filterMatches(items) { const nq = normalizeText(q); return items.filter((a) => normalizeText(a.titulo).includes(nq)); }
    ...
    } catch (e) { return res.status(500).json({ erro: 'Erro na pesquisa do AnimeFire' }); }
};
